		...
		<div>
			<b-table striped hover :items="items" :fields="fields">
				...
				<template #cell(key)="data">
					<a :href="data.item.key" target="_blank">{{ data.item.key }}</a>
				</template>
	...
	<script>
		...
		var app = new Vue({
			...
			data: {
				...
				items: [],
				rows: 0,
				fields: ['id', 'url', {key:'key', label: 'Short URL'}, {key:'created_at', label: 'Date'}],
				page: 1
			},
			mounted() {
				this.init();
			...
			methods: {
				init(){
					axios.post('/history', {page:this.page}).then(({data})=>{
